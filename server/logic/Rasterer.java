...
public class Rasterer {
    ...
    public static RasterResult rasterizeMap(RasterRequest request) {
        Tile[][] grid;
        int upperLeftX = getX(request.depth, request.ullon);
        int upperLeftY = getY(request.depth, request.ullat);
        int lowerRightX = getX(request.depth, request.lrlon);
        int lowerRightY = getY(request.depth, request.lrlat);
        int yTiles = (int) (lowerRightY - upperLeftY) + 1;
        int xTiles = (int) (lowerRightX - upperLeftX) + 1;
        grid = new Tile[yTiles][xTiles];

        for (int y = 0; y < yTiles; y++) {
            for (int x = 0; x < xTiles; x++) {
                grid[y][x] = new Tile(request.depth, x + upperLeftX, y + upperLeftY);
    ...
    public static int getX(int depth, double lon) {
        int xTile = (int) ((lon - ROOT_ULLON) / LON_PER_TILE[depth]);
        return xTile;
    }

    public static int getY(int depth, double lat) {
        int yTile = (int) ((ROOT_ULLAT - lat) / LAT_PER_TILE[depth]);
        return yTile;
    }
